...
class Agent:
    ...
    def getAction(self, state):
        """
        The Agent will receive a GameState (from either {car, capture, sonar}.py) and
...
class AgentState:
    ...
    def __init__( self, startConfiguration, isCar ):
    ...
    def __str__( self ):
        if self.isCar:
            return "Car: " + str( self.configuration )
        else:
    ...
    def copy( self ):
        state = AgentState( self.start, self.isCar )
...
class Grid:
    ...
    via grid[x][y] where (x,y) are positions on a Car map with x horizontal,
    y vertical and the origin (0,0) in the bottom left corner.

    The __str__ method constructs an output that is oriented like a car board.
    """
...
class GameStateData:
    ...
    def initialize( self, layout, numGhostAgents ):
        ...
        self.agentStates = []
        numGhosts = 0
        for isCar, pos in layout.agentPositions:
            if not isCar:
                if numGhosts == numGhostAgents: continue # Max ghosts reached already
                else: numGhosts += 1
            self.agentStates.append( AgentState( Configuration( pos, Directions.STOP), isCar) )
