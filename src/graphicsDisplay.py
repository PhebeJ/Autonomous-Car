...
CAR_COLOR = formatColor(255.0/255.0,255.0/255.0,61.0/255)
CAR_SCALE = 0.5
#car_speed = 0.25
...
class CarGraphics:
    ...
    def drawAgentObjects(self, state):
        ...
        for index, agent in enumerate(state.agentStates):
            if agent.isCar:
                image = self.drawCar(agent, index)
    ...
    def swapImages(self, agentIndex, newState):
        ...
        if newState.isCar:
            image = self.drawCar(newState, agentIndex)
    ...
    def update(self, newState):
        ...
        if self.agentImages[agentIndex][0].isCar != agentState.isCar: self.swapImages(agentIndex, agentState)
        prevState, prevImage = self.agentImages[agentIndex]
        if agentState.isCar:
            self.animateCar(agentState, prevState, prevImage)
        else:
    ...
    def drawCar(self, car, index):
        position = self.getPosition(car)
        screen_point = self.to_screen(position)
        endpoints = self.getEndpoints(self.getDirection(car))

        width = CAR_OUTLINE_WIDTH
        outlineColor = CAR_COLOR
        fillColor = CAR_COLOR

        if self.capture:
            outlineColor = TEAM_COLORS[index % 2]
            fillColor = GHOST_COLORS[index]
            width = CAR_CAPTURE_OUTLINE_WIDTH

        #return [circle(screen_point, CAR_SCALE * self.gridSize,
         #              fillColor = fillColor, outlineColor = outlineColor,
          #             endpoints = endpoints,
           #            width = width)]
        return [image(screen_point, file="./cars/car.gif")]
    ...
    def moveCar(self, position, direction, image):
        screenPosition = self.to_screen(position)
        endpoints = self.getEndpoints( direction, position )
        r = CAR_SCALE * self.gridSize
        moveCircle(image[0], screenPosition, r, endpoints)
        refresh()

    def animateCar(self, car, prevCar, image):
        if self.frameTime < 0:
        ...
        if self.frameTime > 0.01 or self.frameTime < 0:
            start = time.time()
            fx, fy = self.getPosition(prevCar)
            px, py = self.getPosition(car)
            frames = 4.0
            for i in range(1,int(frames) + 1):
                pos = px*i/frames + fx*(frames-i)/frames, py*i/frames + fy*(frames-i)/frames
                self.moveCar(pos, self.getDirection(car), image)
                refresh()
                sleep(abs(self.frameTime) / frames)
        else:
            self.moveCar(self.getPosition(car), self.getDirection(car), image)
        refresh()
    ...
    def updateDistributions(self, distributions):
        ...
        for x in range(len(self.distributionImages)):
            for y in range(len(self.distributionImages[0])):
                ...
                # Fog of war
                color = [0.0,0.0,0.0]
                colors = GHOST_VEC_COLORS[1:] # With Car
...
class FirstPersonCarGraphics(CarGraphics):
    def __init__(self, zoom = 1.0, showGhosts = True, capture = False, frameTime=0):
        CarGraphics.__init__(self, zoom, frameTime=frameTime)
    ...
    def initialize(self, state, isBlue = False):
        ...
        self.isBlue = isBlue
        CarGraphics.startGraphics(self, state)
    ...
    def getPosition(self, ghostState):
        if not self.showGhosts and not ghostState.isCar and ghostState.getPosition()[1] > 1:
            return (-1000, -1000)
        else:
            return CarGraphics.getPosition(self, ghostState)
